...
from mautrix.types import EventType, ImageInfo, MessageType
from mautrix.types.event.message import (
    BaseFileInfo,
    Format,
...
reddit_pattern = re.compile(
    r"((?:https?:)?\/\/)?((?:www|m|old|nm)\.)?((?:reddit\.com|redd\.it))(\/r\/[^/]+\/(?:comments|s)\/[a-zA-Z0-9_\-]+)"
)
...
tiktok_pattern = re.compile(
    r"((?:https?:)?\/\/)?((?:www|m|vm)\.)?((?:tiktok\.com))(\/[@a-zA-Z0-9\-\_\.]+)?(\/video\/)?([a-zA-Z0-9\-\_]+)?"
)
bluesky_pattern = re.compile(
    r"((?:https?:)?\/\/)?((?:www|bsky)\.)?((?:bsky\.app))(\/profile\/[a-zA-Z0-9\-\_\.]+)(\/post\/[a-zA-Z0-9\-\_]+)"
)
...
class SocialMediaDownloadPlugin(Plugin):
    ...
    async def on_message(self, evt: MessageEvent) -> None:
        if (
            ...
            and not (
                self.config["respond_to_notice"]
                and evt.content.msgtype == MessageType.NOTICE
            )
    ...
    async def get_ttdownloader_params(self, tokensDict, url) -> list:
        cookies = {"PHPSESSID": tokensDict["PHPSESSID"]}
        headers = {
    ...
    def get_ttdownloader_tokens(self) -> dict:
        ...
        if response.status_code != 200:
            self.log.warning(
                f"Unexpected status fetching tokens for ttdownloader.com: {response.status_code}"
            )
            return

        token_match = re.search(
            r'<input type="hidden" id="token" name="token" value="([^"]+)"',
            response.text,
        )
    ...
    async def handle_tiktok(self, evt, url_tup):
        ...
        if self.config["tiktok.video"]:
            ...
            cookies, headers, data = await self.get_ttdownloader_params(
                tokensDict, url
            )
            ...
            if response.status != 200:
                self.log.warning(
                    f"Unexpected status sending download request to ttdownloader.com: {response.status}"
                )
                return
            ...
            if response.status != 200:
                self.log.warning(
                    f"Unexpected status fetching video for TikTok URL {url_tup}: {response.status}"
                )
                return
            ...
            uri = await self.client.upload_media(
                media, mime_type=mime_type, filename=file_name
            )
    ...
    async def get_youtube_video_id(self, url):
        if "youtu.be" in url:
            ...
            video_id = urllib.parse.parse_qs(urllib.parse.urlparse(url).query)[
                "v"
            ][0]
    ...
    async def handle_youtube(self, evt, url_tup):
        ...
        if response.status != 200:
            self.log.warning(
                f"Unexpected status fetching video title {query_url}: {response.status}"
            )
            return
        ...
        if self.config["youtube.thumbnail"]:
            ...
            if response.status != 200:
                self.log.warning(
                    f"Unexpected status fetching image {thumbnail_link}: {response.status}"
                )
                return
            thumbnail = await response.read()
            filename = f"{video_id}.jpg"
            uri = await self.client.upload_media(
                thumbnail, mime_type="image/jpeg", filename=filename
            )
    ...
    async def handle_instagram(self, evt, url_tup):
        ...
        if self.config["instagram.info"]:
            await evt.reply(
                TextMessageEventContent(
                    msgtype=MessageType.TEXT,
                    format=Format.HTML,
                    formatted_body=f"""<p>Username: {post.owner_username}<br>Caption: {post.caption}<br>Hashtags: {post.caption_hashtags}<br>Mentions: {post.caption_mentions}<br>Likes: {post.likes}<br>Comments: {post.comments}</p>""",
                )
            )

        if (post.is_video and self.config["instagram.thumbnail"]) or (
            not post.is_video and self.config["instagram.image"]
        ):
            response = await self.http.get(post.url)
            if response.status != 200:
                self.log.warning(
                    f"Unexpected status fetching instagram image {post.url}: {response.status}"
                )
                return
        ...
        if post.is_video and self.config["instagram.video"]:
            response = await self.http.get(
                yarl.URL(post.video_url, encoded=True)
            )
            if response.status != 200:
                self.log.warning(
                    f"Unexpected status fetching instagram video {post.video_url}: {response.status}"
                )
                return
    ...
    async def get_redirected_url(self, short_url: str) -> str:
        async with self.http.get(short_url, allow_redirects=True) as response:
            if response.status == 200:
                ...
                self.log.warning(
                    f"Unexpected status fetching redirected URL: {response.status}"
                )
                return None
    ...
    async def send_image(self, evt, media_url, mime_type, file_name):
        ...
        if response.status != 200:
            self.log.warning(
                f"Unexpected status fetching media {media_url}: {response.status}"
            )
            return
    ...
    async def handle_reddit(self, evt, url_tup):
        ...
        if response.status != 200:
            self.log.warning(
                f"Unexpected status fetching reddit listing {query_url}: {response.status}"
            )
            return
        ...
        if self.config["reddit.info"]:
            await evt.reply(
                TextMessageEventContent(
                    ...
                    format=Format.HTML,
                    body=f"{sub}: {title}",
                    formatted_body=f"""<p><b>{sub}: {title}</b></p>""",
                )
            )
        ...
        if "url_overridden_by_dest" in post_data:
            ...
            if mime_type is None:
                if "is_gallery" in post_data and post_data["is_gallery"]:
                    for media_id, media_info in post_data[
                        "media_metadata"
                    ].items():
                        ...
                        await self.send_image(
                            evt, media_url, mime_type, file_name
                        )
                    return
                elif (
                    ...
                    fallback_url = post_data["secure_media"]["reddit_video"][
                        "fallback_url"
                    ]
                elif (
                    "preview" in post_data
                    and "reddit_video_preview" in post_data["preview"]
                ):
                    fallback_url = post_data["preview"]["reddit_video_preview"][
                        "fallback_url"
                    ]
                else:
                    self.log.warning(
                        f"Unable to determine media url for {query_url}"
                    )
                    return
            ...
            if "image" in mime_type and self.config["reddit.image"]:
                ...
                if response.status != 200:
                    self.log.warning(
                        f"Unexpected status fetching media {download_url}: {response.status}"
                    )
                    return
                ...
                if len(media) == 0:
                    self.log.warning(
                        f"Received 0 bytes when fetching media {download_url}"
                    )
                    return
    ...
    async def handle_bluesky(self, evt, url_tup):
        ...
        async with self.http.get(did_url) as response:
            if response.status != 200:
                self.log.warning(
                    f"Failed to resolve handle {user}: HTTP {response.status}"
                )
                return
        ...
        async with self.http.get(post_url) as response:
            if response.status != 200:
                self.log.warning(
                    f"Failed to fetch post {post_id}: HTTP {response.status}"
                )
                return
            ...
            if embed:
                if "images" in embed:
                    ...
                    for image in embed["images"]:
                        ...
                        mime_type = (
                            mimetypes.guess_type(media_url)[0] or "image/jpeg"
                        )
                    ...
                    self.log.info(
                        f"Video URL: {playlist_url}, Thumbnail URL: {thumbnail_url}"
                    )

                    if playlist_url and self.config["bluesky.video"]:
                        media_bytes = await self.download_m3u8_file(
                            playlist_url
                        )
                        if not media_bytes:
                            self.log.warning(
                                f"Failed to download video from {playlist_url}"
                            )
                            return
                        ...
                        uri = await self.client.upload_media(
                            media_bytes, mime_type=mime_type, filename=file_name
                        )
                        await self.client.send_file(
                            evt.room_id,
                            url=uri,
                            info=BaseFileInfo(
                                mimetype=mime_type, size=len(media_bytes)
                            ),
                            file_name=file_name,
                            file_type=MessageType.VIDEO,
                        )
                    ...
                    if thumbnail_url and self.config["bluesky.thumbnail"]:
                        mime_type = (
                            mimetypes.guess_type(thumbnail_url)[0]
                            or "image/jpeg"
                        )
                        file_name = f"{post_id}_thumbnail.jpg"
                        await self.send_image(
                            evt, thumbnail_url, mime_type, file_name
                        )
    ...
    async def download_m3u8_file(self, m3u8_url: str):
        async with self.http.get(m3u8_url) as response:
            if response.status != 200:
                self.log.warning(
                    f"Failed to fetch playlist: {m3u8_url} — HTTP {response.status}"
                )
                return b""
        ...
        if "#EXT-X-STREAM-INF" in playlist:
            ...
            if not next_m3u8:
                self.log.warning(
                    f"No variant found in master playlist: {m3u8_url}"
                )
                return b""
        ...
        for i, url in enumerate(segment_urls):
            async with self.http.get(url) as segment_response:
                if segment_response.status != 200:
                    self.log.warning(
                        f"Failed to download segment {i + 1}: {url} — HTTP {segment_response.status}"
                    )
                    continue
                segment_data.append(await segment_response.read())
                if i % 10 == 0:
                    self.log.debug(
                        f"Downloaded segment {i + 1}/{len(segment_urls)}"
                    )
